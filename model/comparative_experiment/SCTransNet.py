...
# 原文中的feature mapping 映射为编码器对应的h w,c不变
...
class Attention_org(nn.Module):
    ...
    def forward(self, emb1, emb2, emb3, emb4, emb_all):
        ...
        # 你在处理的是图像数据，通常来自 CNN 或卷积模块，特征形状是：
        # [B, C, H, W]  # batch, 通道, 高, 宽
        # 但注意力机制（尤其是Transformer中的）期望输入是：
        # 序列形式，像NLP中的词序列一样：[B, N, D]，其中 N = H×W是“patch数量”，D是维度
        # 并且要用多头注意力（Multi - HeadAttention），那就还需要把D拆成head × head_dim所以你需要一个“桥梁”操作 —— 就是这个rearrange。
        ...
        #论文中是用的层归一化，而且是再进入      q1 = self.q1(self.mhead1(emb1))操作之前就完成了
        ...
        _, _, c4, _ = q4.shape
        _, _, c, _ = k.shape
        # self.KV_size其实也就是cat操作后的得到的通道数，下面是qk操作
        ...
        # 转化为图像数据，之前是序列数据，所以需要用rearrange
...
# 就是对某一个维度做均值方差，然后算这一维度的归一化？再缩放平移，说到底就是对某一个维度操作，这两个层归一化的区别就是带不带平移的可学习β
...
class LayerNorm3d(nn.Module):
    ...
    def forward(self, x):
        h, w = x.shape[-2:]
        # to_3d因为层归一化是为了3d张量准备的，所以先转化为3d张量
...
class eca_layer_2d(nn.Module):
    def __init__(self, channel, k_size=3):
        ...
        # 上面池化为 b c 1 1，下面卷积对通道维度单独卷积
        self.conv = nn.Sequential(
            # Conv1d要求的是输入的形状是 [B, 1, c]，所以这里先转置一下
...
class SCTransNet(nn.Module):
    ...
    def forward(self, x):
        # 实际上有五个residual block，下面五行都是，但是用了不同名字
        ...
        if self.deepsuper:
            ...
            if self.mode == 'train':
                return (torch.sigmoid(out), torch.sigmoid(d0), torch.sigmoid(gt2),torch.sigmoid(gt3), torch.sigmoid(gt4),torch.sigmoid(gt5))
            else:
