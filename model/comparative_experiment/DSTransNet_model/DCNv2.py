...
class DCNv2(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
        ...
        self.weight = nn.Parameter(
            ...
            torch.empty(out_channels, in_channels // groups, *self.kernel_size) # 修正了这里的错误，in_channels应该除以groups
        )
...
def main():
    ...
    # block = DCNv2(in_channels=3, out_channels=10, kernel_size=3, stride=1, padding=1, groups=3) # groups与输入channel和输出channel都有关系
    # block = DCNv2(in_channels=3, out_channels=10, kernel_size=3, stride=1, padding=1,deformable_groups=3) # deformable_groups只和输入channel有关系
    block = DCNv2(in_channels=3, out_channels=3, kernel_size=5, stride=2, padding=None)
    output_tensor = block(input_tensor)
    print(output_tensor.size())

    # input_tensor = torch.randn(1, 3, 256, 256)
    # block = DCNv2(in_channels=3, out_channels=3, kernel_size=3, stride=1, padding=1)
    # output_tensor = block(input_tensor)
    # print(output_tensor.size())
